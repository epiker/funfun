...
(function () {
    ...
    UploadImage.prototype = {
        ...
        getInsertList: function () {
            var i, data, list = [],
                align = getAlign(),
                replace = editor.getOpt('imageUrlReplace')
                prefix = editor.getOpt('imageUrlPrefix');
            for (i = 0; i < this.imageList.length; i++) {
                data = this.imageList[i];
                list.push({
                    src: prefix + data.url.replace(replace, ''),
                    _src: prefix + data.url.replace(replace, ''),
                    title: data.title,
                    alt: data.original,
                    floatStyle: align
                });
    ...
    OnlineImage.prototype = {
        ...
        pushData: function (list) {
            ...
            urlReplace = editor.getOpt('imageManagerUrlReplace'),
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    img.width = 113;
                    img.setAttribute('src', urlPrefix + list[i].url.replace('/'+urlReplace, '') + (list[i].url.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
                    img.setAttribute('_src', urlPrefix + list[i].url.replace('/'+urlReplace, ''));
